...
gpu_failure_flag_filename = "./gpu-already-failed.txt"
...
if args.gpu_failure:
    ...
    if not os.path.exists(gpu_failure_flag_filename):

        with open(gpu_failure_flag_filename,"w") as fd:
            pass

        print(f"[{args.name}] Simulating GPU failure.", flush=True)

        # TODO : some code to simulate GPU failure
